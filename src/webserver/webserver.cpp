...
WebServer::WebServer(
    ...
    srcDir_ = new char[resourcePath.size()];
    memcpy(srcDir_, &resourcePath[0], resourcePath.size());
    ...
    // 初始化数据库连接池
    SqlConnPool::Instance()->Init("local-mysql", sqlPort, sqlUser, sqlPwd, dbName, connPoolNum);
